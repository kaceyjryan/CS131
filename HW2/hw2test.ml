...
(* My Test Cases *)
  ...
  (*  Provided theoretical grammar from hw2 spec :: Same Grammar used for sample test cases
      Rewritten so I could run my test cases separately or with the sample ones in one file *)
...
let test_1 = ((parse_prefix test_grammar accept_all ["5"; "-"; "2"]) = 
  Some([(Expr, [N Term; N Binop; N Expr]); (Term, [N Num]); (Num, [T"5"]); 
        (Binop, [T"-"]); (Term, [N Num]); (Num, [T"2"])], [])) (* FAILS *)

let test_2 = ((parse_prefix test_grammar accept_all ["5"; "--"]) = 
  Some([(Expr, [N Term; N Incrop]); (Term, [N Num]); (Num, [T"5"]); (Incrop, [T"--"])], []) ) (* FAILS *)

let test_3 = ((parse_prefix awkish_grammar accept_all ["5"]) = 
 Some ([(Expr, [N Term]); (Term, [N Num]); (Num, [T "5"])], []) ) (* SUCCEEDS *)
